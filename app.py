...
def descargar_archivo_dinamico(nombre_archivo):
    # Definimos la lista de archivos que tenemos permitido descargar (por seguridad)
    archivos_permitidos = [
...
def enviaractividad():
    ...
    try:
        ...
        try:
            ...
            base_name = f"actividad_{user['carrera']}_{user['numero_control']}_{user['apellido_paterno']}_{user['apellido_materno']}_{user['nombres']}_{actividad_num}_{asig}"
...
def login():
    if request.method == 'POST':
        ...
        try:
            ...
            flash(f'{username} _¡Bienvenido de vuelta a tu clase!', 'success')
            return redirect(url_for('hello_pm1'))
